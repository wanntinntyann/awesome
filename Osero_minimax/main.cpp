/*オセロプログラムを作ってみました。キー入力でやります。ミニマックス法とアルファベータ法
を調べて実装しました。先読みしてる最中にパスする局面があったら少し動作が怪しいような気が
します。ここまでで修正すべき個所があったらコメントして教えてください。*/
...
int tansaku(int you, int I, int lim, int& x, int& y)
{
	...
	if (lim >= YOMI) {
		...
		if ((I == white && teban == 6) || (I == black && teban == 5)) {
			value = -2000;
			//親ノードが-1000の時はリターンさせない
			kizyun[lim] = -2000;
		}
		else {
			value = 2000;
			//親ノードが1000の時はリターンさせない
			kizyun[lim] = 2000;
		}
		for (int i = 1; i <= N; i++) {
			for (int j = 1; j <= N; j++) {
				if (AI_hantei(j, i, you, I, lim)) {
					...
					if ((I == black && teban == 5) || (I == white && teban == 6)) {
						//枝切り処理
						if (child > kizyun[lim] || kizyun[lim - 1] == 2000) {
							kizyun[lim] = child;
						}
						//親ノードが1000でない時はリターンする
						else if (kizyun[lim] > kizyun[lim - 1] && lim > 0) {
							modosu(you, I, lim);
							return kizyun[lim];
						}
